...
async function main() {
  ...
  const state = await page.evaluate(() => {
    ...
    return {
      ...
      lane: document.getElementById("laneLabel").textContent,
      modalHidden: document.getElementById("modal").classList.contains("hidden"),
      eventCount: JSON.parse(localStorage.getItem("neon-lane-dash-events") || "[]").length,
      colored,
    };
  });
  ...
  if (!state.lane) failures.push("Game did not update lane label.");
  ...
  console.log("Neon Lane Dash smoke passed.");
}
