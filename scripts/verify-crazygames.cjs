...
assert(
  platform.includes("adsAllowed") &&
    platform.includes('params.get("ads") === "0"') &&
    platform.includes('state.provider === "playgama"') &&
    platform.includes('params.get("ads") === "1"'),
  "Ad requests must be gated for self-hosted builds and allowed in Playgama SDK context."
);
assert(!html.toLowerCase().includes("watch ads") && !game.toLowerCase().includes("watch ads"), "Build should not contain watch-ad copy.");
...
console.log("Neon Lane Dash platform SDK verification passed.");
