...
export async function onRequestPost({ request, env }) {
  if (!env.NLD_EVENTS) return json({ ok: false, error: "Event store unavailable" }, 503);
  try {
    ...
    await Promise.all(keys.map((key) => increment(env.NLD_EVENTS, key)));
    return json({ ok: true });
  } catch {
...
export function onRequestGet() {
  return json({ ok: true, service: "Neon Lane Dash event collector" });
}
